...
class ConfigManager:
	...
	def setConfigValInFile(
		...
		if os.path.dirname(configFile) != additionalConfigDir:
			print(
				"Error: failed to set configuration value; config file given not in expected config directory. Only specify name of file, not the full path.",
				file=sys.stderr
			)
			exit(1)
