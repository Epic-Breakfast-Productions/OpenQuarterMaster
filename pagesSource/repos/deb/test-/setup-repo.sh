...
# get GPG key
curl -s --compressed "https://deployment.openquartermaster.com/repos/deb/test-/KEY.gpg" | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/oqm_ppa.gpg >/dev/null
#add repo to list
curl -s --compressed "https://deployment.openquartermaster.com/repos/deb/test-/deb_list_file.list" | sudo tee /etc/apt/sources.list.d/oqm_file.list
