...
class CharacteristicsUtils:
	...
	def __get_banner(cls, characteristicsYaml) -> BannerCache | None:
		if 'banner' not in characteristicsYaml:
			return None
		return BannerCache(
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_BANNER_TEXT', characteristicsYaml['banner']['text']),
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_BANNER_TEXTCOLOR', characteristicsYaml['banner']['textColor']),
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_BANNER_BACKGROUNDCOLOR', characteristicsYaml['banner']['backgroundColor']),
		)
	...
	def __get_run_by(cls, characteristicsYaml) -> RunByCache | None:
		if 'runBy' not in characteristicsYaml:
			return None
		return RunByCache(
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_RUNBY_NAME', characteristicsYaml['runBy']['name']),
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_RUNBY_EMAIL', characteristicsYaml['runBy']['email']),
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_RUNBY_PHONE', characteristicsYaml['runBy']['phone']),
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_RUNBY_WEBSITE', characteristicsYaml['runBy']['website']),
			ImageUtils.get_image(
				os.getenv('CHARACTERISTICS_VAL_RUNBY_LOGOIMG', characteristicsYaml['runBy']['logoImg']),
				os.getenv('CHARACTERISTICS_RUNBY_IMG_DIR', "/")
			),
			ImageUtils.get_image(
				os.getenv('CHARACTERISTICS_VAL_RUNBY_BANNERIMG', characteristicsYaml['runBy']['bannerImg']),
				os.getenv('CHARACTERISTICS_RUNBY_IMG_DIR', "/")
			),
		)
	...
	def get_characteristics_cache(cls) -> CharacteristicsCache:
		...
		output = CharacteristicsCache(
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_TITLE', characteristicsYaml['title']),
			DataUtils.sanitizeInput('CHARACTERISTICS_VAL_MOTD', characteristicsYaml['motd']),
