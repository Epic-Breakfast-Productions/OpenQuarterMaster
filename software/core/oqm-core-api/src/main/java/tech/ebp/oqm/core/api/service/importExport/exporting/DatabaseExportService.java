...
public class DatabaseExportService {
	...
	private static void archiveFile(File outputFile, Path dirToArchiveAsPath) throws IOException {
		...
		}
		{
			...
			try (
				...
				Files.walkFileTree(
					dirToArchiveAsPath, new SimpleFileVisitor<>() {
						@Override
	...
	public File exportDataToBundle(DataExportOptions options) throws IOException {
		...
		{//writing out data files
			...
			Map<OqmMongoDatabase, CompletableFuture<Void>> databaseFutures = new HashMap<>();
			{
				...
				for (OqmMongoDatabase db : dbList) {
					...
					databaseFutures.put(
						db, this.threadContext.withContextCapture(
							CompletableFuture.supplyAsync(()->{
								String dbId = db.getId().toHexString();
								StopWatch sw = StopWatch.createStarted();
								
								CompletableFuture<Void> future = CompletableFuture.allOf(
									this.threadContext.withContextCapture(CompletableFuture.supplyAsync(()->{
		...
		archiveFile(outputFile, dirToArchiveAsPath);
