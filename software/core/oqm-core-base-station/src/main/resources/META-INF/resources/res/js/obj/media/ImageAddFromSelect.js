...
export class ImageAddFromSelect extends PageUtility {
	...
	static imageUploadInput = $("#imageSearchSelectAdd");
	...
	static resetImageAdd() {
		console.debug("Resetting image add.");
		...
		ImageAdd.reset(ImageAddFromSelect.imageUploadInput);
		console.debug("Done resetting image add.");
	}
	...
	static {
		...
		ImageAddFromSelect.imageAddImageForm.submit(function (ev) {
			...
			ImageAdd.getImageData(ImageAddFromSelect.imageUploadInput, ev)
				.then(function (imageDataStr) {
					console.log("Got image data: ", imageDataStr);
					let addData = new FormData();

					addData.append("fileName", ImageAdd.Input.getFileName(ImageAddFromSelect.imageUploadInput));
