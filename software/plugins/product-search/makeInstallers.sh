...
mkdir -p "$buildDir/$debDir/etc/oqm/static/plugin/external-item-search/"
mkdir -p "$buildDir/$debDir/etc/oqm/serviceConfig/plugin/external-item-search/"
...
#install -m 755 -D "$srcDir/uiEntry.json" "$buildDir/$debDir/etc/oqm/static/plugin/external-item-search/"
install -m 755 -D "$srcDir/plugin-ext_item_search-config.list" "$buildDir/$debDir/etc/oqm/serviceConfig/plugin/external-item-search/"
install -m 755 -D "$srcDir/30-plugin-ext_item_search.json" "$buildDir/$debDir/etc/oqm/config/configs/"
#install -m 755 -D "$srcDir/plugin-ext_item_search-proxy-config.json" "$buildDir/$debDir/etc/oqm/proxyConfig.d/"

serviceFile="oqm-plugin-ext_item_search.service"
...
Upstream-Name: Open QuarterMaster - Plugin - External Item Search
...
mkdir -p /etc/oqm/serviceConfig/plugin/external-item-search/files/

if [ ! -f "/etc/oqm/serviceConfig/plugin/external-item-search/user-config.list" ]; then
	cat <<EOF >> "/etc/oqm/serviceConfig/plugin/external-item-search/user-config.list"
...
systemctl daemon-reload
# Remove docker image
if [[ "$(docker images -q oqm-plugin-ext_item_search 2> /dev/null)" != "" ]]; then
        docker rmi oqm-plugin-ext_item_search
        echo "Removed docker image."
else
        echo "Docker image was already gone."
fi
if [ $( docker ps -a | grep oqm-core-api | wc -l ) -gt 0 ]; then
        docker rm oqm-plugin-ext_item_search
