...
mkdir -p "$buildDir/$debDir/etc/oqm/static/plugin/image-search/"
mkdir -p "$buildDir/$debDir/etc/oqm/serviceConfig/plugin/image-search/"
...
#install -m 755 -D "$srcDir/uiEntry.json" "$buildDir/$debDir/etc/oqm/static/plugin/image-search/"
install -m 755 -D "$srcDir/plugin-image_search-config.list" "$buildDir/$debDir/etc/oqm/serviceConfig/plugin/image-search/"
install -m 755 -D "$srcDir/30-plugin-image_search.json" "$buildDir/$debDir/etc/oqm/config/configs/"

serviceFile="oqm-plugin-image_search.service"
...
mkdir -p /etc/oqm/serviceConfig/plugin/image-search/files/

if [ ! -f "/etc/oqm/serviceConfig/plugin/image-search/user-config.list" ]; then
	cat <<EOF >> "/etc/oqm/serviceConfig/plugin/image-search/user-config.list"
# Add your own config here.
# Configuration here will override those in plugin-image_search-config.list
...
systemctl daemon-reload
# Remove docker image
if [[ "$(docker images -q oqm-plugin-image_search 2> /dev/null)" != "" ]]; then
        docker rmi oqm-plugin-image_search
        echo "Removed docker image."
else
        echo "Docker image was already gone."
fi
if [ $( docker ps -a | grep oqm-plugin-image-search | wc -l ) -gt 0 ]; then
        docker rm oqm-plugin-image_search
