...
public class InvItemBumper2 extends ObjectSchemaVersionBumper<InventoryItem> {
	...
	private ObjectNode adjustStored(ObjectNode oldStored, String storageBlock){
		oldStored.remove("id");
	...
	protected SingleUpgradeResult bumpObjectSchema(ObjectNode oldObj) {
		...
		switch (oldType) {
			case "AMOUNT_SIMPLE":
				createdObjectsResults.put(AmountStored.class, newStoredList);
			...
			case "AMOUNT_LIST":
				...
				break;
			case "TRACKED":
				createdObjectsResults.put(UniqueStored.class, newStoredList);
		...
		for (Iterator<String> it = oldStorageMap.fieldNames(); it.hasNext(); ) {
			...
			switch (oldType) {
				case "AMOUNT_SIMPLE": {
					...
					if (newStored != null) {
						newStoredList.add(adjustStored(newStored, curBlock));
					}
				}
				break;
				case "AMOUNT_LIST": {
					...
					if (storedList != null) {
						storedList.forEach((JsonNode newStored)->{
							newStoredList.add(adjustStored((ObjectNode) newStored, curBlock));
						});
					}
				}
				break;
