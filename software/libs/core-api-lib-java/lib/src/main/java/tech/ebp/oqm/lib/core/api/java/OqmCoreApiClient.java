...
public class OqmCoreApiClient {
	...
	private OqmCredentials defaultCredentials;
	...
	private HttpRequest.Builder setupRequest(
		...
		reqBuilder.uri(this.getConfig().getBaseUri().resolve(path));
	...
	public CompletableFuture<HttpResponse<ObjectNode>> getApiServerHealth() {
		return this.getHttpClient()
			.sendAsync(
				this.setupRequest(null, "/q/health").build(),
