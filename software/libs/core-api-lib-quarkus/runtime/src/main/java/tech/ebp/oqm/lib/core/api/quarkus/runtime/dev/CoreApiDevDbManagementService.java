...
public class CoreApiDevDbManagementService {
	...
	public String resetAndPopulateDb(String db) {
		...
		{// item - bulk
			log.info("Creating bulk item: {}", this.newItem("BULK")
													.set(
														"storageBlocks",
														objectMapper.createArrayNode()
															.add(objectMapper.createObjectNode().set("storageBlock", storageBlock1.get("id")))
															.add(objectMapper.createObjectNode().set("storageBlock", storageBlock2.get("id")))
													));
